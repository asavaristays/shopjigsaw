...
add_action('wp_enqueue_scripts', function(){ wp_add_inline_style('rj-shop-branding', '
/* Premium compact product-grid CTAs. */
.woocommerce ul.products {
...
.woocommerce ul.products li.product {
  ...
  background:
    ...
    #f6f0e7 !important;
  box-shadow: 0 20px 56px -50px rgba(30,30,30,.44) !important;
}

.woocommerce ul.products li.product:hover {
  transform: translateY(-3px) !important;
  box-shadow: 0 26px 64px -54px rgba(30,30,30,.46) !important;
}
...
.woocommerce ul.products li.product .astra-shop-thumbnail-wrap {
  ...
  background:
    radial-gradient(circle at 30% 24%, rgba(255,255,255,.74), transparent 34%),
    linear-gradient(145deg, rgba(239,231,221,.88), rgba(179,192,164,.18)) !important;
}

.woocommerce ul.products li.product a img {
  max-height: 218px !important;
  mix-blend-mode: normal !important;
  filter: saturate(1.02) contrast(1.01) drop-shadow(0 14px 18px rgba(30,30,30,.08)) !important;
}
...
.woocommerce ul.products li.product .price {
  ...
  color: rgba(30,30,30,.76) !important;
  font-size: 13px !important;
  font-weight: 900 !important;
}
...
.woocommerce ul.products li.product .added_to_cart {
  ...
  border: 1px solid rgba(184,107,75,.24) !important;
  border-radius: 999px !important;
  background: rgba(184,107,75,.10) !important;
  color: #9a563b !important;
...
.woocommerce ul.products li.product .added_to_cart:hover {
  ...
  border-color: rgba(184,107,75,.44) !important;
  background: #b86b4b !important;
  color: #fff !important;
  box-shadow: 0 14px 30px -24px rgba(184,107,75,.82) !important;
}

.woocommerce ul.products li.product .button.added {
  background: rgba(120,134,107,.14) !important;
  border-color: rgba(120,134,107,.28) !important;
...
.woocommerce ul.products li.product .added_to_cart {
  ...
  color: #1d647a !important;
  border-color: rgba(29,100,122,.22) !important;
  background: rgba(29,100,122,.08) !important;
}
...
.single-product .rj-story-panel {
  ...
  box-shadow: 0 24px 70px -58px rgba(30,30,30,.42);
}
...
.single-product .rj-story-art img {
  ...
  max-height: 430px;
  object-fit: contain;
  filter: drop-shadow(0 22px 24px rgba(30,30,30,.10));
}
...
add_action('woocommerce_after_single_product_summary', function () {
    ...
    $short_name = preg_replace('/\s*[–-]\s*\d+\s*pieces?.*$/i', '', $name);
    $short_name = trim($short_name) ?: $name;
    $piece_count = '';

    if (preg_match('/(\d+)\s*pieces?/i', $name, $matches)) {
        $piece_count = $matches[1] . ' pieces';
    }
    ...
    echo '<div>';
    echo '<p class="rj-story-kicker">Product experience</p>';
    echo '<h2>' . esc_html($short_name) . ' is designed as more than a puzzle.</h2>';
    echo '</div>';
    echo '<p>Use this puzzle as a calm focus ritual, a screen-free family table activity, or a thoughtful premium gift. Each section below gives buyers more context before they add it to cart.</p>';
    echo '</div>';

    echo '<div class="rj-story-panel-grid">';
    $panels = array(
        array('01', 'Improve Focus', 'Train attention through slow visual matching, pattern recognition and immersive hands-on concentration.'),
        array('02', 'Relax Your Mind', 'Create a quiet pause from digital fatigue with a tactile activity that naturally slows the pace.'),
        array('03', 'Family Connection', 'Bring people around one table for conversation, collaboration and meaningful screen-free time.'),
        array('04', 'Premium Gifting', 'Artwork-led presentation and thoughtful product design make it suitable for personal and corporate gifting.'),
    );
    foreach ($panels as $panel) {
        echo '<article class="rj-story-panel">';
        echo '<strong>' . esc_html($panel[0]) . '</strong>';
        echo '<h3>' . esc_html($panel[1]) . '</h3>';
        echo '<p>' . esc_html($panel[2]) . '</p>';
        echo '</article>';
    }
    echo '</div>';
    ...
    if ($story_image) {
        echo '<img src="' . esc_url($story_image) . '" alt="' . esc_attr($short_name . ' lifestyle and product detail') . '" loading="lazy" />';
    }
    echo '</div>';
    echo '<div class="rj-story-copy">';
    echo '<p class="rj-story-kicker">Why it works</p>';
    echo '<h2>A mindful object for slow evenings and sharper attention.</h2>';
    echo '<p>Premium puzzles need room to communicate the full experience: artwork, material quality, cognitive engagement and the emotional reason to buy.</p>';
    echo '<ul>';
    echo '<li>Ideal for focused solo sessions, mindful breaks and quiet weekends.</li>';
    echo '<li>Works well for families, professionals, gifting buyers and puzzle enthusiasts.</li>';
    echo '<li>Designed to feel premium on the table and meaningful after completion.</li>';
    echo '</ul>';
    echo '</div>';
    echo '</div>';
    ...
    echo '<div>';
    echo '<p class="rj-story-kicker">Product details</p>';
    echo '<h2>Everything a buyer needs before checkout.</h2>';
    echo '</div>';
    ...
    foreach ($details as $detail) {
        echo '<div class="rj-story-detail">';
        echo '<span>' . esc_html($detail[0]) . '</span>';
        echo '<strong>' . esc_html($detail[1]) . '</strong>';
        echo '</div>';
    }
    echo '</div>';
    echo '</div>';
